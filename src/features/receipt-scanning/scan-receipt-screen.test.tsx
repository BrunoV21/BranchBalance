...
describe('ScanReceiptScreen', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    jest.mocked(useGroup).mockReturnValue({ state: { data: { group: { name: 'Trip', currency: 'EUR' } } } } as never);
  ...
  it('keeps manual entry available when the native development build is missing', async () => {
    ...
    mockGetStatus.mockResolvedValue({ state: 'module_unavailable', engine: 'paddle_ocr', modelBundleVersion: null, safeMessage: 'Receipt scanning requires the BranchBalance development build.' });
  ...
  it('turns split native OCR blocks into an in-memory amount prefill', async () => {
    const replace = jest.fn();
    jest.mocked(useRouter).mockReturnValue({ replace } as never);
    mockGetStatus.mockResolvedValue({ state: 'ready', engine: 'paddle_ocr', modelBundleVersion: 'ppocr-v5-mobile-latin-2026-07-20', safeMessage: 'Receipt scanning is ready.' });
    ...
    await waitFor(() => expect(mockSetReceiptDraft).toHaveBeenCalledWith(expect.objectContaining({ description: 'CAFE CENTRAL', amount: '43.27', expenseDate: '2026-07-20' })));
    expect(replace).toHaveBeenCalledWith({ pathname: '/groups/[owner]/[repo]/expenses/new', params: { owner: 'owner', repo: 'trip' } });
  });
});
