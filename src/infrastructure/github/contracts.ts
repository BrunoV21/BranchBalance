import type { AccountProfile, CommittedMutation, CurrencyCode, DiscoveredGroup, ExpenseFile, Group, GroupCommitSlice, GroupFile, InvitationDiscoveryResult, IsoInstant, KnownGroupType, PendingGroupCreation, RemoteGroupSnapshot, RepositoryRef, SettlementLedgerFile, SettlementLedgerState, SettlementPayment, SettlementValidationBasis, SpendingPlan, WritableExpense } from '@/domain/types';
...
export interface GitHubGateway {
  ...
  createExpense(repository: RepositoryRef, expense: WritableExpense, signal?: AbortSignal, effectiveType?: KnownGroupType): Promise<CommittedMutation<ExpenseFile>>;
  readExpense(repository: RepositoryRef, id: string, signal?: AbortSignal): Promise<ExpenseFile | null>;
  updateExpense(repository: RepositoryRef, current: ExpenseFile, expense: WritableExpense, signal?: AbortSignal, effectiveType?: KnownGroupType): Promise<CommittedMutation<ExpenseFile>>;
  deleteExpense(repository: RepositoryRef, current: ExpenseFile, signal?: AbortSignal): Promise<CommittedMutation<null>>;
  updateSpendingPlan(repository: RepositoryRef, current: GroupFile, next: SpendingPlan | null, signal?: AbortSignal, effectiveType?: KnownGroupType): Promise<CommittedMutation<GroupFile>>;
