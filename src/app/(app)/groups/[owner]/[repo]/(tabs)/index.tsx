...
export default function GroupOverviewScreen() {
  ...
  const fuelMonth = snapshot?.analytics?.type === 'fuel' ? snapshot.analytics.fuel.selectedMonth : null;
  ...
  return <SafeAreaView style={[styles.screen, { backgroundColor: colors.background }]}>
    <FlatList data={snapshot?.expenses ?? []} keyExtractor={(item) => item.expense.id} contentContainerStyle={styles.content}
      ...
      ListHeaderComponent={<View style={styles.header}><Title eyebrow={snapshot && groupType ? groupContextLabel(groupType, snapshot.group.name) : snapshot?.group.name ?? 'BranchBalance'}>Group overview</Title>
        ...
        {groupType === 'trip' && snapshot?.spending?.budget ? <Pressable accessibilityRole="button" accessibilityLabel={`Open spending analytics${snapshot.spending.analytics.pace ? `. ${formatMoney(Math.abs(snapshot.spending.analytics.pace.deltaMinor), snapshot.group.currency)} ${snapshot.spending.analytics.pace.direction} even budget pace` : ''}.`} onPress={viewSpending}><BudgetSummaryCard compact summary={snapshot.spending} currency={snapshot.group.currency} /></Pressable> : null}
        {groupType === 'fuel' && fuelMonth ? <Pressable accessibilityRole="button" accessibilityLabel="Open Fuel spending analytics" onPress={viewSpending}><Card><Body muted>{fuelMonth.month} Fuel summary</Body><View style={styles.fuelSummary}><View><Body>Paid this month</Body><Text style={[styles.summaryValue, { color: colors.text }]}>{formatMoney(fuelMonth.paidMinor, snapshot!.group.currency)}</Text></View><View><Body>Fill-ups</Body><Text style={[styles.summaryValue, { color: colors.text }]}>{fuelMonth.expenseCount}</Text></View></View><Body>{fuelMonth.applicableLimitMinor === null ? 'No monthly limit applies.' : fuelMonth.status === 'over' ? `${formatMoney(Math.abs(fuelMonth.remainingMinor!), snapshot!.group.currency)} over the monthly limit.` : `${formatMoney(fuelMonth.remainingMinor!, snapshot!.group.currency)} remains this month.`}</Body><Body muted>{formatLitres(fuelMonth.representedVolumeMl)} represented · {fuelMonth.expensesWithVolume} of {fuelMonth.expenseCount} expenses include litres · {formatMoney(fuelMonth.explicitDiscountMinor, snapshot!.group.currency)} explicit discounts</Body></Card></Pressable> : null}
      ...
      renderItem={({ item }) => <Pressable accessibilityRole="button" accessibilityLabel={`Open ${item.expense.description}`} onPress={() => open(item)}><Card>
        ...
        <View style={styles.metadataGrid}>
          <View style={styles.metadataCell}>{groupType === 'fuel' ? <MetadataItem icon={<CategoryIcon category="transport" size={18} />} label={item.expense.type_data ? formatLitres(item.expense.type_data.volume_millilitres) : 'Amount only'} accessibilityLabel={item.expense.type_data ? `${formatLitres(item.expense.type_data.volume_millilitres)} of Fuel` : 'Amount-only Fuel expense'} /> : <MetadataItem icon={<CategoryIcon category={item.expense.category ?? 'uncategorized'} size={18} />} label={categoryLabel(item.expense.category ?? 'uncategorized')} accessibilityLabel={`Category: ${categoryLabel(item.expense.category ?? 'uncategorized')}`} />}</View>
